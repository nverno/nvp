...
// switches to control garbage collection policy, and a switch to
// enable optimization. The optimization flag is ignored by the
// reference compiler.
//
// All flags that can be set on the command line should be defined
// here; otherwise, it is necessary to pollute test drivers for
// components of the compiler with declarations of extern'ed debugging
// flags to satisfy the linker. The exceptions to this rule are
// yy_flex_debug and cool_yydebug, which are defined in files
