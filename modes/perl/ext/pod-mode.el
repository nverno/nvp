;;; pod-mode.el --- Major mode for editing .pod-files -*- lexical-binding: t; -*-
...
(eval-when-compile
  (require 'cl-lib)
  (defvar pod-font-lock-keywords-1)
  (defvar perldoc-modules-alist)
    ...
    (save-match-data
      (setf pod-weaver-section-keywords
            (cl-loop for col in collectors
                  ...
                  do (progn
                       (setq cmd (cl-getf col 'command)
                             new-cmd (cl-getf col 'new_command)
...
`pod-enable-weaver-collector-keywords'."
  (with-current-buffer buffer
    (pod-enable-weaver-collector-keywords (cl-getf weaver-config 'collectors))
...
(declare-function eproject-maybe-turn-on "eproject")
(declare-function perldoc-modules-alist "perldoc")
