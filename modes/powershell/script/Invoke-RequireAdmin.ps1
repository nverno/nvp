...
function Get-UNCFromPath {
  ...
  if ($Path.Contains([io.path]::VolumeSeparatorChar)) {
    $psdrive = Get-PSDrive -Name $Path.Substring(0, 1) `
      -PSProvider 'FileSystem'

    # Is it a mapped drive?
    if ($psdrive.DisplayRoot) {
      $Path = $Path.Replace(
        $psdrive.Name + [io.path]::VolumeSeparatorChar, `
           $psdrive.DisplayRoot)
    }
  }
...
function Invoke-RequireAdmin {
  ...
  if (-not (Test-IsAdmin)) {
    ...
    try {    
      $process = Start-Process PowerShell.exe -PassThru -Verb Runas `
         -Wait -ErrorAction SilentlyContinue -WorkingDirectory $pwd `
...
function Start-NoPrompt {
  <# 
  .SYNOPSIS
   Run script without prompts/window popus. With argument $asAdmin,
   read password from ~/password.bin and run script elevated.
  #>
  ...
  if ($asAdmin) {
    ...
    # -ArgumentList $params
    Start-Process powershell -WindowStyle hidden -Verb `
      runas -ArgumentList $params
  } else {
