  ...
  (if (not nvp-trace--active)
    ...
    (pcase type
      ('grp (when (or (null nvp-trace--type)
              ...
              (setq nvp-trace--current
                    (append name (nvp:as-list nvp-trace--current)))))
...
(define-minor-mode nvp-trace-minor-mode
  "Tracing minor mode."
...
(defun nvp-trace-kill ()
  "Untrace everything and quit `nvp-trace-minor-mode'."
  (interactive)
...
(defun nvp-trace--read-group (prompt &optional force-read untrace)
...
(defun nvp-trace-group (group)
  ...
  (interactive (list (nvp-trace--read-group "Trace: " current-prefix-arg)))
  (dolist (fn (cdr group))
...
(defun nvp-untrace-group (funcs)
  ...
  (interactive (list (nvp-trace--read-group "Untrace: " current-prefix-arg t)))
  (dolist (fn funcs)
    (untrace-function fn)
